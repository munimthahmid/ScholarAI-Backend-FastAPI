...
async def test_enhanced_pdf_collection():
    ...
    try:
        ...
        print(f"   📋 Regular batch: {len(processed_papers)}/{len(batch_papers)} papers retained")
        
        # Test parallel batch processing
        processed_papers_parallel = await pdf_processor.process_papers_batch_parallel(batch_papers, batch_size=2)
        print(f"   🚀 Parallel batch: {len(processed_papers_parallel)}/{len(batch_papers)} papers retained")
        ...
        arxiv_paper = {"url": "https://arxiv.org/abs/2301.12345", "title": "ArXiv Extraction Test"}
        arxiv_id = pdf_collector._extract_arxiv_id(arxiv_paper)
        print(f"   🔬 ArXiv ID extraction: {arxiv_id}")
        
        # Test bioRxiv extraction
        biorxiv_paper = {"url": "https://www.biorxiv.org/content/10.1101/2021.01.01.425001v1", "title": "bioRxiv Test"}
        biorxiv_id = pdf_collector._extract_biorxiv_id(biorxiv_paper)
        print(f"   🧬 bioRxiv ID extraction: {biorxiv_id}")
        
        # Test PMC extraction
        pmc_paper = {"url": "https://www.ncbi.nlm.nih.gov/pmc/articles/PMC1234567/", "title": "PMC Test"}
