...
async def storage_health_check():
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"B2 storage service unavailable: {str(e)}")


@router.get("/stats", response_model=StorageStatsResponse, summary="Get Storage Statistics")
...
@router.delete("/files/all", response_model=BulkDeleteResponse, summary="Delete All PDF Files")
...
async def delete_paper_pdf(
    ...
    try:
        ...
        if not paper_dict:
            raise HTTPException(status_code=400, detail="At least one paper identifier must be provided")
        ...
        if success:
            ...
            raise HTTPException(status_code=404, detail="PDF not found for the given paper identifiers")
            
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Failed to delete paper PDF: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to delete paper PDF: {str(e)}")
...
async def get_paper_pdf_url(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get paper PDF URL: {str(e)}")
    ...
    try:
        ...
        return {
            ...
            "paper": {
                ...
                "processing_status": "success" if processed_paper.get("pdfContentUrl") else "failed"
            }
        }
        
    except Exception as e:
        logger.error(f"Failed to process paper PDF: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to process paper PDF: {str(e)}")
...
async def generate_content_report(_: None = Depends(get_admin_dependencies)):
    ...
    try:
        ...
        return {
            ...
            "storage_efficiency": {
                "avg_file_size_mb": round(stats.get("total_size_mb", 0) / max(stats.get("total_files", 1), 1), 2),
                "largest_category": max(file_categories, key=file_categories.get) if file_categories else "none"
            }
        }
...
async def initialize_storage(_: None = Depends(get_admin_dependencies)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to initialize B2 storage: {str(e)}")


@router.post("/test/search-with-pdf", summary="Test Paper Search with PDF Processing")
async def test_search_with_pdf(
    query: str = Query(..., description="Search query"),
    limit: int = Query(default=5, ge=1, le=20, description="Number of papers to search"),
    _: None = Depends(get_admin_dependencies)
):
    ...
    try:
        ...
        if pdf_stats["papers_with_pdf_urls"] > 0:
            pdf_stats["b2_upload_success_rate"] = round(
                (pdf_stats["papers_with_b2_urls"] / pdf_stats["papers_with_pdf_urls"]) * 100, 2
            )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to test search with PDF: {str(e)}") 