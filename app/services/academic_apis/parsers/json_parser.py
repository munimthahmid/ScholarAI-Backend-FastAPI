...
class JSONParser:
    ...
    def parse_openalex_paper(paper: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "authorships" in paper:
            result["authors"] = JSONParser._extract_openalex_authors(paper["authorships"])
    ...
    @staticmethod
    def _extract_openalex_authors(authorships: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def parse_core_paper(paper: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "journals" in paper and isinstance(paper["journals"], list) and paper["journals"]:
            result["venueName"] = paper["journals"][0].get("title", "")
        elif "journal" in paper:
            if isinstance(paper["journal"], dict):
                result["venueName"] = paper["journal"].get("title", paper["journal"].get("name", ""))
            else:
        ...
        date_fields = ["publishedDate", "datePublished", "publicationDate", "yearPublished"]
        for date_field in date_fields:
            if date_field in paper and paper[date_field]:
                if date_field == "yearPublished" and isinstance(paper[date_field], (int, str)):
                    ...
                    if len(date_str) == 4 and date_str.isdigit():  # Just year
                        ...
                        result["publicationDate"] = date_str.split('T')[0]  # Remove time part if present
                break
        ...
        if "language" in paper:
            if isinstance(paper["language"], dict):
                result["language"] = paper["language"].get("name", paper["language"].get("code", ""))
            else:
                result["language"] = str(paper["language"])
        
        # Extract subject/topics
        if "topics" in paper and isinstance(paper["topics"], list):
            result["topics"] = [topic.get("name", str(topic)) if isinstance(topic, dict) else str(topic) 
                              for topic in paper["topics"]]
        elif "subjects" in paper:
            result["topics"] = paper["subjects"] if isinstance(paper["subjects"], list) else [paper["subjects"]]
                ...
                if isinstance(author, dict):
                    ...
                    if "name" in author:
                        ...
                        if "given" in author or "firstName" in author:
                            name_parts.append(author.get("given", author.get("firstName", "")))
                        if "family" in author or "lastName" in author:
                            name_parts.append(author.get("family", author.get("lastName", "")))
                        if name_parts:
                    ...
                    if "orcid" in author:
                        ...
                        if isinstance(orcid, str):
                            # Clean ORCID format
                            author_info["orcid"] = orcid.replace("https://orcid.org/", "").replace("http://orcid.org/", "")
                    ...
                    if "affiliation" in author:
                        ...
                        elif isinstance(author["affiliation"], list) and author["affiliation"]:
                    ...
                    elif "affiliations" in author and isinstance(author["affiliations"], list) and author["affiliations"]:
    ...
    def parse_unpaywall_paper(paper: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "oa_locations" in paper and paper["oa_locations"]:
            ...
            for location in oa_locations:
                if (location.get("host_type") == "repository" and 
    ...
    def parse_europepmc_paper(paper: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "authorList" in paper and "author" in paper["authorList"]:
            result["authors"] = JSONParser._extract_europepmc_authors(paper["authorList"]["author"])
    ...
    @staticmethod
    def _extract_europepmc_authors(authors_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def _extract_doaj_authors(authors_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def parse_base_paper(paper: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "oa" in paper:
            result["isOpenAccess"] = paper["oa"] == 1 or paper["oa"] == "1" or paper["oa"] is True
