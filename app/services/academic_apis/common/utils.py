...
def extract_doi(data: Dict[str, Any]) -> Optional[str]:
    ...
    doi_fields = [
        "doi", "DOI", "dc:identifier", "prism:doi",
        "externalIds.DOI", "identifiers.doi", "arxiv_doi"
    ]
    ...
    for field in doi_fields:
        ...
        if value:
            # Clean DOI prefixes
            clean_doi = str(value).replace("https://doi.org/", "").replace("http://dx.doi.org/", "")
...
def extract_date(data: Dict[str, Any]) -> Optional[str]:
    ...
    """
    date_fields = [
        "publicationDate", "publishedDate", "date", "published",
        "year", "publicationYear", "datePublished", "created",
        "updated", "prism:publicationDate"
    ]
...
def parse_authors(authors_data: Union[List[Dict], List[str], str]) -> List[Dict[str, Any]]:
    """
...
def extract_metrics(data: Dict[str, Any]) -> Dict[str, int]:
    ...
    citation_fields = ["citationCount", "cited_by_count", "citedByCount", "num_cited_by"]
