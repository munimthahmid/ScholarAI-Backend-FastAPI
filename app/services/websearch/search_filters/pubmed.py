...
class PubMedFilter(BaseSearchFilter, DateFilterMixin):
    ...
    def _add_domain_filter(self, filters: Dict[str, Any], domain: str):
        ...
        if is_medical:
            ...
            filters["publication_types"] = ["Journal Article", "Clinical Trial", "Review"]
            
            # Add specific filters based on subdomain
            if "clinical" in domain_lower:
                filters["publication_types"].extend(["Clinical Study", "Randomized Controlled Trial"])
            
            if "genetics" in domain_lower or "molecular" in domain_lower:
                filters["publication_types"].extend(["Research Support, N.I.H., Extramural"])
    ...
    def get_filter_info(self) -> Dict[str, Any]:
        ...
        base_info.update({
            ...
            "supported_domains": [
                "Medicine", "Biology", "Clinical Research", "Biomedical",
                "Neuroscience", "Genetics", "Pharmacology", "Pathology"
            ],
            "publication_types": [
