...
class CrossrefFilter(BaseSearchFilter, DateFilterMixin):
    ...
    def _add_domain_filter(self, filters: Dict[str, Any], domain: str):
        ...
        if domain:
            ...
            if any(term in domain_lower for term in ["computer science", "engineering", "technology"]):
                # Focus on journal articles for technical fields
                filters["type"] = "journal-article"
                filters["has_abstract"] = True
                
            elif any(term in domain_lower for term in ["medicine", "biology", "clinical", "health"]):
                # Medical and biological research - focus on peer-reviewed articles
                filters["type"] = "journal-article"
                filters["has_abstract"] = True
                filters["has_full_text"] = True
                
            elif any(term in domain_lower for term in ["physics", "chemistry", "mathematics"]):
                # Physical sciences - include both journal articles and proceedings
                filters["type"] = "journal-article"
                filters["has_abstract"] = True
                
            elif any(term in domain_lower for term in ["social", "economics", "psychology", "sociology"]):
