...
class PDFCollectorService:
    ...
    async def collect_pdf(self, paper: Dict[str, Any]) -> Optional[bytes]:
        ...
        logger.warning(f"❌ Failed to collect PDF for: {paper.get('title', 'Unknown')[:50]}")
        return None
    ...
    async def _try_direct_urls(self, paper: Dict[str, Any]) -> Optional[bytes]:
        ...
        for field in ['pdfUrl', 'pdf_url', 'pdfLink', 'pdf_link', 'fullTextUrl', 'fulltext_url']:
            url = paper.get(field)
        ...
        if isinstance(links, list):
            for link in links:
                if isinstance(link, dict):
                    ...
                    if url and ('pdf' in url.lower() or link.get('type', '').lower() == 'pdf'):
                        urls_to_try.append(url)
    ...
    async def _try_alternative_urls(self, paper: Dict[str, Any]) -> Optional[bytes]:
        ...
        if doi:
            ...
            if 'nature.com' in paper.get('url', ''):
                ...
                alternative_urls.append(f"https://link.springer.com/content/pdf/{doi}.pdf")
            elif 'ieee.org' in paper.get('url', ''):
                alternative_urls.append(f"https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber={doi.split('/')[-1]}")
    ...
    async def _collect_biorxiv_pdf(self, biorxiv_id: str) -> Optional[bytes]:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                pdf_link = soup.find('a', {'class': 'btn-pdf'}) or soup.find('a', href=re.compile(r'\.pdf$'))
