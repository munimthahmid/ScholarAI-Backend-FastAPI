...
try:
    ...
    ArxivClient = BioRxivClient = COREClient = CrossrefClient = DBLPClient = DOAJClient = \
    EuropePMCClient = OpenAlexClient = SemanticScholarClient = BASESearchClient = PlaceholderClient
...
async def test_single_client(
    ...
    try:
        papers: List[Dict[str, Any]] = await client_instance.search_papers(query=query, limit=limit)
        papers_found_count = len(papers)
        if papers_found_count > 0:
            success = True
            message = f"{Colors.GREEN}SUCCESS (Found {papers_found_count} paper(s)){Colors.ENDC}"
            logger.info(f"{client_name} found {papers_found_count} paper(s). First title: {papers[0].get('title', 'N/A')}")
        else:
            success = False # Technically success, but no results found for this query
            message = f"{Colors.YELLOW}OK (Found 0 papers for query '{query}'){Colors.ENDC}"
            logger.warning(f"{client_name} found 0 papers for query '{query}'.")

    except NotImplementedError:
        success = False
        message = f"{Colors.RED}ERROR (Client not available due to import error){Colors.ENDC}"
...
async def test_biorxiv_client(client_factory, client_name: str) -> Tuple[str, bool, int, str]:
    """
        ...
        for query in test_queries:
            ...
            try:
                ...
                if papers_count > 0:
                    ...
                    logger.info(f"{client_name} found {papers_count} paper(s) for '{query}'. First title: {papers[0].get('title', 'N/A')}")
                    await client_instance.close()
...
async def main():
    ...
    if success_count > 0:
        ...
        print(f"🎯 SUCCESS RATE: \033[92m{success_count}/{total_active} ({success_rate:.1f}%)\033[0m")
    ...
    if warning_count > 0:
        print(f"   • Investigate {warning_count} API(s) returning no results - may need query tuning")
